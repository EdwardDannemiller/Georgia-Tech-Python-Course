...
def find_net_force(forceList):
    xMag = 0
    yMag = 0
    
    for force in forceList:
        xMag += force.get_horizontal()
        yMag += force.get_vertical()
    
    totalAng = round(degrees(atan2(yMag, xMag)), 1)
    totalMag = round((xMag**2 + yMag**2)**(1/2), 1)
    result = Force(totalMag, totalAng)
    return result
